...
router.get('/me', auth, async (req, res) => {
  try {
    const profile = await Profile.findOne({
      user: req.user.id
      // here we bring in name and avatar from user model
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // if profile is not found, we create it
...
// @desc    Update profile, user and posts
// @access  Private
router.put(
  '/experience',
  [
...
router.get('/github/:username', async (req, res) => {
  try {
    ...
    request(options, (error, response, boddy) => {
      ...
      return res.json(JSON.parse(boddy));
    });
  } catch (err) {
    console.error(err.message);
    return res.status(500).send('Server Error');
